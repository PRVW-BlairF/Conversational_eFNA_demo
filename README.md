...
## Real-time conversation capture (GitHub Pages static demo)
- The static demo now supports browser microphone capture using Web Speech API (`SpeechRecognition` / `webkitSpeechRecognition`).
- Click **Start listening** to transcribe speech in real time and auto-update FNA fields as transcript segments arrive.
- Click **Stop listening** to pause capture.
- If browser support is unavailable, guided demo mode remains available as fallback.
...
## Known limitations
- Static mode supports browser microphone capture where Web Speech API is available; otherwise guided simulator mode is used.
