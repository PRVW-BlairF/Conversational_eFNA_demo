...
import { Mic, Square, Upload, FileDown, RotateCcw } from 'lucide-react';

export function HeroHeader({ completion, onGuidedDemo, onReset }: { completion: number; onGuidedDemo: () => void; onReset: () => void }) {
  return (
    <div className="rounded-xl2 bg-copilot-teal text-copilot-inverse p-6 shadow-card">
      <div className="flex justify-between items-start gap-4">
        <div>
          ...
          <div className="flex gap-2 mt-3 text-xs">
            ...
            <span className="px-3 py-1 rounded-full border border-copilot-border">Adviser controlled</span>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mt-5">
        ...
        <button onClick={onGuidedDemo} className="px-4 py-2 rounded-full bg-white text-copilot-teal font-medium flex items-center gap-2"><FileDown size={16} />Advance guided demo</button>
        <button onClick={onReset} className="px-4 py-2 rounded-full border border-copilot-border text-copilot-inverse flex items-center gap-2"><RotateCcw size={16} />Reset</button>
      </div>
